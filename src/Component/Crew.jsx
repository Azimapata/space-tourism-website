...
const Crew = () => {
  // State to keep track of the currently selected crew member
  const [selectedCrew, setSelectedCrew] = useState('douglas');

  // Object containing details of each crew member
  const crewDetails = {
    douglas: {
      role: 'COMMANDER',
      name: 'DOUGLAS HURLEY',
  ...
  // Function to update the state with the selected crew member
  const handleToggle = (crew) => {
    setSelectedCrew(crew);
  };

  // Destructuring the details of the currently selected crew member
    ...
    <div className='crew w-full h-screen bg-cover bg-center overflow-hidden'>
      ...
      <div className='text-center text-white lg:flex'>
        ...
        <div className='pl-0 lg:pl-80 lg:pt-32'>
          ...
          {/* Crew Description with line breaks */}
          ...
          {/* Toggle Buttons to switch between crew members */}
          <div className='flex justify-center space-x-4 lg:space-x-10 pt-16 md:pt-12 lg:pt-32 lg:pr-56'>
            {/* Button for Douglas */}
            <button 
              onClick={() => handleToggle('douglas')} 
              className={`w-2.5 md:w-3.5 h-2.5 md:h-3.5 rounded-full ${selectedCrew === 'douglas' ? 'bg-white' : 'bg-gray-600'}`}>
            </button>
            
            {/* Button for Mark */}
            <button 
              onClick={() => handleToggle('mark')} 
              className={`w-2.5 md:w-3.5 h-2.5  md:h-3.5 rounded-full ${selectedCrew === 'mark' ? 'bg-white' : 'bg-gray-600'}`}>
            </button>

            {/* Button for Victor */}
            <button 
              onClick={() => handleToggle('victor')} 
              className={`w-2.5 md:w-3.5 h-2.5  md:h-3.5 rounded-full ${selectedCrew === 'victor' ? 'bg-white' : 'bg-gray-600'}`}>
            </button>

            {/* Button for Anousheh */}
            <button 
              onClick={() => handleToggle('anousheh')} 
              className={`w-2.5 md:w-3.5 h-2.5  md:h-3.5 rounded-full ${selectedCrew === 'anousheh' ? 'bg-white' : 'bg-gray-600'}`}>
            </button>
          </div>
        </div>
